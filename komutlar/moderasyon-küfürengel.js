...
if (!args[0])  {
    ...
    .setTitle('Başarısız')
    .setDescription(`<a:onaylamak:825688895286280232> Bunumu Arıyorsun? \n ${prefix}küfür-engel aç/kapat`)
      return message.channel.send(küfürcu0k)

  }   
  if (args [0] == 'aç') {
    db.set(`reklamengel_${message.guild.id}`, '<a:onaylamak:825688895286280232> **Küfür Engel Aktif!**')
    let lus = await db.fetch(`kufurE_${message.guild.id}`)
    
    const reklamengelcim = new Discord.MessageEmbed()
    .setTitle('Başarılı')
      .setColor("#6278c5")
    .setDescription('<a:onaylamak:825688895286280232> **Küfür Engel Başarıyla Açıldı**')
  ...
  if (args [0] == 'kapat') {
   ...
   const küfürengelcim22 = new Discord.MessageEmbed()
    .setTitle('Başarılı')
    .setDescription('<a:onaylamak:825688895286280232> **Küfür Engel Başarıyla Kapatıldı**')
